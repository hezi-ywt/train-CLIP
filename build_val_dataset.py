...
from data_loader.tag_tool import year_tag, rating_tag, quality_tag, meta_tags_filter, extract_special_tags
...
def format_meta_data(record):
    ...
    formatted_record = {
        ...
        'character_tags': record['tag_string_character'].split(" ") if record['tag_string_character'] else [],
        
        'tag_string_artist': record['tag_string_artist'],
        'artist_tags': record['tag_string_artist'].split(" ") if record['tag_string_artist'] else [],
        ...
        "copyright_tags": record['tag_string_copyright'].split(" ") if record['tag_string_copyright'] else []
    }
...
if __name__ == '__main__':
    ...
    random_ids = random.sample(all_ids, 1000000)
    counts = 1000
    
    image_data_path = "/mnt/data/danbooru_newest-webp-4Mpixel-all/images_out"


    save_path = "/mnt/data/clip_val_dataset/danbooru_newest-webp-4Mpixel-all_1000"
    ...
    for id in tqdm(random_ids):
        ...
        record = gen_meta_by_id(df, id, danbooru_flo2_caption_ft_long)
        caption = add_base_caption(record)
        image_path = os.path.join(image_data_path, f"{id}.webp")
        if os.path.exists(image_path):
            image = Image.open(image_path)
            image.save(os.path.join(save_path, f"{id}.webp"))
            with open(os.path.join(save_path, f"{id}.txt"), "w") as f:
                f.write(caption)
            count += 1
            if count >= counts:
                break
